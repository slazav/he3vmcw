...
main(){
  ...
  {
    int i;
    double J0a[3], J0b[3], J0[3], JD[3];
    fill_jg0_nt_(J0a, J0b, n0, &t0, gn, &gt);
    fill_jgd_nt_(JD, n0, &t0, gn, &gt);
    FOR(i) J0[i] = J0a[i]/2 + J0b[i];
    check3("J0 vs JD", J0, JD, E, false);
  }
  ...
  {
    int i;
    double T0a[3], T0b[3], T0[3], TD[3];
    fill_tg0_nt_(T0a, T0b, n0, &t0, gn, &gt, ggn, &ggt);
    fill_tgd_nt_(TD, n0, &t0, gn, &gt, ggn, &ggt);
    FOR(i) T0[i] = T0a[i]/2 + T0b[i];
    check3("T0 vs TD", T0, TD, E, false);
  }
