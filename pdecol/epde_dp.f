C  Algorithm 688: EPDCOL: a more efficient PDECOL code:
C  ACM Transactions on Mathematical Software (TOMS), 17, 153-166 (1991)
C  https://dl.acm.org/citation.cfm?id=108558
...
C-----------------------------------------------------------------------
     ...
     *                 IW11,IW12,IW13,IW14,IW15,IW16,IW17,IW18
      COMMON /IOUNIT/ LOUT
      DIMENSION WORK(KORD+4*NPDE+NCPTS*(3*KORD+2)
     *   +NPDE*NCPTS*(MAXDER+6)+(NPDE**2)*(13+KORD*(KORD-NCC)*NINT)),
...
C-----------------------------------------------------------------------
      DIMENSION Y(N0,MAXDER+1),YMAX(N0),ERROR(N0),SAVE1(N0),SAVE2(N0),
...
C FORTRAN FUNCTIONS USED..   NONE
C-----------------------------------------------------------------------
      DIMENSION C(NPDE,NCPTS),R(NPDE,1),V(NPDE,NCPTS)
